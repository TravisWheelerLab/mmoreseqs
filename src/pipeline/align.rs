...
pub struct AlignArgs {
    ...
    /// The path to the P7HMM used for alignment. This will either be `query_path`,
    /// or a new file generated by running hmmbuild on `query_path`.
    #[clap(skip)]
    ...
    let profiles = match profiles {
        ...
        None => {
            match args.query_format {
                FileFormat::Fasta => {
                    build_hmm_from_fasta(&args.query_path, &args.hmm_path, args.num_threads)?;
                }
                FileFormat::Stockholm => {
                    build_hmm_from_stockholm(&args.query_path, &args.hmm_path, args.num_threads)?;
                }
                FileFormat::Hmm => {
                    // no-op
                }
            ...
            let hmms = parse_hmms_from_p7hmm_file(&args.hmm_path)?;
